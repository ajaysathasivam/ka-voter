...
export default function VoterInfoPage() {
    const [data, setData] = useState<any[]>([]); // API response
    ...
    useEffect(() => {
        const fetchStates = async () => {
            try {
                ...
                setData(res);
                setStates(res.map((s: any) => s.name));
            } catch (err) {
                console.error(err);
    ...
    useEffect(() => {
        if (selectedState) {
            const stateObj = data.find((s) => s.name === selectedState);
            setDistricts(stateObj?.districts.map((d: any) => d.name) || []);
            setSelectedDistrict("");
            setAssemblies([]);
            setSelectedAssembly("");
        }
    ...
    useEffect(() => {
        if (selectedDistrict && selectedState) {
            const stateObj = data.find((s) => s.name === selectedState);
            const districtObj = stateObj?.districts.find((d: any) => d.name === selectedDistrict);
            setAssemblies(districtObj?.assemblies.map((a: any) => a.name) || []);
            setSelectedAssembly("");
        }
    }, [selectedDistrict, selectedState, data]);

    // Fetch candidates for selected assembly
    const fetchCandidates = async () => {
        if (!selectedState || !selectedDistrict || !selectedAssembly) {
            toast("Please select state, district, and assembly first.");
            return;
        }

        try {
            // Find selected assembly _id
            const stateObj = data.find((s) => s.name === selectedState);
            const districtObj = stateObj?.districts.find((d: any) => d.name === selectedDistrict);
            const assemblyObj = districtObj?.assemblies.find((a: any) => a.name === selectedAssembly);

            // Replace with your real API
            const res = await apiCall({
                url: `${API_URL}/api/admin/candidates`,
                method: "POST",
                body: {
                    stateId: stateObj?._id,
                    assemblyId: assemblyObj?._id,
            ...
            console.log(err, 'err');
            toast("Failed to fetch candidates asfd");
        }
    };
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-start px-4 py-12 bg-gray-50">
            <div className="w-full max-w-xl space-y-6">
                ...
                {/* District Dropdown */}
                <div>
                    <label className="block mb-1 text-sm">Select District</label>
                    <Select
                ...
                <div>
                    ...
                    >
                        <SelectTrigger className="w-full">
                            <SelectValue placeholder={selectedDistrict ? "Select Assembly" : "Select district first"} />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="grid grid-cols-1 gap-4 mt-6">
                        {filteredCandidates.map((candidate, idx) => (
                            <Card key={idx} className="p-4 shadow">
                                ...
                                <Link
                                    className="w-full bg-[#9e7b7f] text-center rounded-lg py-2 hover:bg-[#8b6b6f] text-white"
                                    href={`/vote/${candidate._id}`}
                                >
                                    View Candidate
                                </Link>
                            </Card>
